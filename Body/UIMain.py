...
def Ai_voice():
    global engine
    global voice
    open('Database\\temp_chat.txt', 'w').close()
    if os.path.exists("Body\\voice\\"):
        ...
        if voice=="0": Brain.Aibrain.Assign_Brain("chat_log_Marco.txt")
        elif voice=="1": Brain.Aibrain.Assign_Brain("chat_log_Maria.txt")
    else:
        voice=0
        Brain.Aibrain.Assign_Brain("chat_log_Marco.txt")
...
def Set_aivoice(command):
    ...
    if "maria" in command:
        ...
        file.close()
        Brain.Aibrain.Assign_Brain("chat_log_Maria.txt")
    elif "marco" in command:
        file=open("Body\\voice\\voice.txt", "w")
        file.write("0")
        engine = pyttsx3.init('sapi5')                 #sapi5:- Application programming interface (API) for voice recog. and synthesis.  
        voices = engine.getProperty('voices')          #Gets the current value of a property.
...
def takeCommand(alm=0): 
    ...
    Gui_Marshall.terminalOutput("\nYou : "+ command)
    return command   

def speak(audio):
    '''To speak output by Desktop Assistant'''
    global engine
    Gui_Marshall.terminalOutput("\nAI : "+ audio)
    engine.say(audio)
...
def wait_M(): 
    ...
    while True:
        ...
        if c=="none":
            continue
        elif "wake up" in c or "hello" in c or "marco" in c or "maria" in c:
            break
def sleep_M():
...
def gautoTools(command):
    ...
    elif "turn off" in command or "stop" in command and "automatic" in command or "stop" in command and "automate" in command:
...
def Win_handle(command):
        ...
        elif "close folder" in command or "close current window" in command or "close window" in command or "close this window" in command:
            keyboard.press_and_release("Ctrl + w")
        ...
        else: speak(f"Sorry, I can't do that.")

def TaskExe(command):
    if "maria" in command and "switch" in command or "marco" in command and "switch" in command:
    ...
    elif "today's date" in command or "tell" in command and "date" in command or "what is date" in command:
        date()

    elif "current time" in command or "tell" in command and "time" in command or "what is time" in command:
        current_time() 
        
    elif "today's day" in command or "tell" in command and "day" in command or "what is day" in command:
        todays_day()
...
class MainThread(QThread):
    ...
    def Executor(self):
        ...
        greets() 
        while True:
            self.command = takeCommand()
            if self.command=="none":
                continue
            else:
                TaskExe(self.command)
                
class Gui_Start(QMainWindow):
    def __init__(self):
        super().__init__()
        self.marshall_ui = Ui_MarshalUI()
        self.marshall_ui.setupUi(self)
        self.startFunc()
        self.marshall_ui.QUIT.clicked.connect(self.close) 

    def startFunc(self):      
        self.marshall_ui.move_Marshall = QtGui.QMovie("Body\\siri_1.gif")
        self.marshall_ui.Marshall.setMovie(self.marshall_ui.move_Marshall)
        self.marshall_ui.move_Marshall.start()

        self.marshall_ui.move_Mlogo = QtGui.QMovie("Body\\M.gif")
        self.marshall_ui.Mlogo.setMovie(self.marshall_ui.move_Mlogo)
        self.marshall_ui.move_Mlogo.start()

        self.marshall_ui.move_Mlogo2 = QtGui.QMovie("Body\\M2.gif")
        self.marshall_ui.Mlogo2.setMovie(self.marshall_ui.move_Mlogo2)
        self.marshall_ui.move_Mlogo2.start()
        
        self.marshall_ui.move_Mouth = QtGui.QMovie("Body\\mouth.gif")
        self.marshall_ui.Mouth.setMovie(self.marshall_ui.move_Mouth)
        self.marshall_ui.move_Mouth.start()  

        self.marshall_ui.move_Face = QtGui.QMovie("Body\\AIface.gif")
        self.marshall_ui.Face.setMovie(self.marshall_ui.move_Face)
        self.marshall_ui.move_Face.start() 

        self.marshall_ui.move_loading = QtGui.QMovie("Body\\loading.gif")
        self.marshall_ui.loading.setMovie(self.marshall_ui.move_loading)
        self.marshall_ui.move_loading.start()

        start_M.start()

    def terminalOutput(self, command):
        self.marshall_ui.plainTextEdit.appendPlainText(command)
 

start_M = MainThread()
Gui_App = QApplication(sys.argv)
Gui_Marshall = Gui_Start()
Gui_Marshall.show()
